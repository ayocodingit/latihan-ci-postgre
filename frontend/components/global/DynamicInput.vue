  ...
  <div>
    <select class="form-control" v-model="input"
      ...
      <template v-if="!is_object">
        <option :value="item" v-for="(item, $index) in options.filter(el => el != ignoreParam)" :key="$index">{{ item }}</option>
      </template>
      <template v-if="is_object">
        <option :value="item.id" v-for="(item, $index) in options.filter( el => el.id != ignoreParam )" :key="$index">{{ item.name ? item.name : item.text }}</option>
      </template>
...
export default {
  ...
  props: ['form', 'field', 'options', 'placeholder', 'hasLainnya','ignoreParam', 'hasSemua'],
  methods: {
    reset() {
  ...
  created() {
    this.$bus.$on('clear-dropdown', this.reset);
  },
};
</script>
