<template>
  <tr>
    ...
    <td>
      <div class="badge badge-white" style="text-align:left; padding:10px"># {{item.nomor_sampel}}
        <br>{{item.deskripsi}}
      </div>
    </td>
    <td nowrap>
  ...
  export default {
    ...
    methods: {
      async deleteData(item, usia) {
        ...
        if (isConfirm) {
          try {
            await this.$axios.delete(`v1/register/mandiri/${item.register_id}/${item.pasien_id}`);
