<template>
  <tr>
    ...
    <td>
      {{ item.metode_ekstraksi }}
    </td>
    <td>
      <nuxt-link :to="`reagen-ekstraksi/update/${item.id}`" class="mb-1 btn btn-primary btn-sm">
  ...
  export default {
    ...
    methods: {
      async deleteData(item) {
        ...
        if (isConfirm) {
          this.$axios.delete(`v1/reagen-ekstraksi/${item.id}`)
            .then((response) => {
              swal.fire(
                'Berhasil Menghapus Data',
                'Reagen Ekstraksi Berhasil dihapus',
                'success'
              );
              bus.$emit('refresh-ajaxtable', 'master-reagen-ekstraksi')
            })
            .catch((error) => {
              swal.fire(
                'Terjadi Kesalahan',
