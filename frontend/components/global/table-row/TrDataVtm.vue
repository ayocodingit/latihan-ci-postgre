<template>
  <tr>
    ...
    <td>
      <nuxt-link :to="`jenis-vtm/update/${item.id}`" class="mb-1 btn btn-primary btn-sm">
  ...
  export default {
    ...
    methods: {
      async deleteData(item) {
        ...
        if (isConfirm) {
          this.$axios.delete(`v1/jenis-vtm/${item.id}`)
            .then((response) => {
              swal.fire(
                'Berhasil Menghapus Data',
                'Jenis VTM Berhasil dihapus',
                'success'
              );
              bus.$emit('refresh-ajaxtable', 'master-vtm')
            })
            .catch((error) => {
              swal.fire(
                'Terjadi Kesalahan',
