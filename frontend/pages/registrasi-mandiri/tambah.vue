<template>
  <div class="wrapper wrapper-content">
    <portal to="title-name">
      Registrasi Baru Pasien Mandiri
    </portal>
        ...
        <form @submit.prevent="submit" @keydown="form.onKeydown($event)">
              ...
              <div class="col-md-8 row">
                <div class="col-md-8">
                  <vue-bootstrap-typeahead ref="ref_search_pasien" placeholder="Cari Pasien Terdaftar"
                    ...
                    :data="data_pasien" backgroundVariant="white" textVariant="dark">
                    <template slot="suggestion" slot-scope="{ data }">
                      <strong>Nama:</strong>
                      {{ data.name }}
                      <small v-if="data.birth_date">
                        {{ data && data.age ? `/ Usia ${data.age} tahun` : ''}}
                      </small>
                      <br>
                ...
                <div class="col-md-4">
                  <button @click="onSelectedPasien($event)" class="btn btn-md btn-primary block full-width"
                    type="button">
          ...
          <Ibox title="Penerimaan atau Pengambilan Sampel">
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8 col-lg-6">
                ...
                <div class="form-check form-check-inline">
                  <label>
                    <input v-model="form.reg_sumberpasien" value="Other" type="radio" v-on:click="otherCategoryHandler">
                    <span>Isian</span>
                  </label>
                </div>
              </div>
            ...
            <div class="form-group row" v-if="form.reg_sumberpasien=='Other'">
              <div for="" class="col-md-3 col-lg-2 flex-text-center"></div>
              <div class="col-md-8 col-lg-6">
          ...
          <Ibox title="Alamat Pasien">
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8 col-lg-6">
                <textarea class="multisteps-form__input form-control" type="text" name="reg_alamat" required
                  ...
                  :class="{ 'is-invalid': form.errors.has('reg_alamat') }" />
            ...
            <div class="form-group row">
              ...
              <div class="input-group col-md-3 col-lg-2 flex-text-center">
                ...
                <input class="form-control" type="text" name="reg_domisilirw" v-model="form.reg_rw" :class="{ 'is-invalid':form.errors.has('reg_rw') }" />
              </div>
            ...
            <div class="form-group row mt-4">
              ...
              <div class="col-md-8 col-lg-6" :class="{ 'is-invalid': form.errors.has('reg_kota_id') }">
                <vue-bootstrap-typeahead
                  ...
                  v-model="kota_text"
                />
                <input id="input-kota-kabupaten" type="text" v-model="form.reg_kota_id" required />
          ...
          <Ibox title="Riwayat Kunjungan">
            <div class="form-group row">
              ...
              <div class="col-md-8 col-lg-6">
                <div class="form-check form-check-inline" v-for="index of 10" v-bind:key="index"
                  ...
                  <label>
                    <input name="reg_kunjungan_ke" :id="`reg_kunjungan_ke-${index}`" :value="index"
                      v-model="form.reg_kunjungan_ke" type="radio">
                    <span>Ke-{{index}}</span>
                  </label>
                </div>
              </div>
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8 col-lg-6" :class="{ 'is-invalid': form.errors.has('reg_tanggalkunjungan') }">
            ...
            <div class="form-group row">
              ...
              <div class="col-md-8 col-lg-6">
                <input class="form-control" type="text" name="reg_rsfasyankes"
                  placeholder="Nama Rumah Sakit / Fasyankes" v-model="form.reg_rsfasyankes" :class="{ 'is-invalid':form.errors.has('reg_rsfasyankes') }" />
          ...
          <Ibox title="Identitas Sampel">
            <div class="form-group row">
              <div class="col-md-3 col-lg-2 flex-text-center col-form-div" for="sampel">
                Nomor Sampel
                <span style="color:red">*</span></div>
                <div class="col-md-8 col-lg-6">
                  <input class="form-control" type="text" name="pen_sampel" v-model="form.reg_sampel"
                    placeholder="Nomor Sampel" :class="{ 'is-invalid':form.errors.has(`reg_sampel`) }" required />
  ...
  export default {
    ...
    methods: {
      ...
      async onSelectedPasien(item) {
        ...
        this.form.reg_tempatlahir = item.tempat_lahir
        ...
        let provinsi = item.address_province_code && this.dataProvinsi ? this.dataProvinsi.find(el => el.id === item.address_province_code) : null
        this.$refs.ref_provinsi.inputValue = provinsi ? provinsi.nama : ''

        await this.getKota(item.address_province_code)
        let kota = await item.address_district_code && this.dataKota ? this.dataKota.find(el => el.id === item.address_district_code) : null
        this.$refs.ref_kota.inputValue = kota ? kota.nama : ''
        
        await this.getKecamatan(item.address_district_code)
        let kecamatan = await  item.address_subdistrict_code && this.dataKecamatan ? this.dataKecamatan.find(el => el.id === item.address_subdistrict_code) : null
        this.$refs.ref_kecamatan.inputValue = kecamatan ? kecamatan.nama : ''

        await this.getKelurahan(item.address_subdistrict_code)
        let kelurahan =  await item.address_village_code && this.dataKelurahan ? this.dataKelurahan.find(el => el.id === item.address_village_code) : null
      ...
      async getNoreg() {
        const resp = await this.$axios.get('/v1/register/noreg')
        const { result } = resp.data || ''
        this.form.reg_no = result
      },
      initForm() {
      ...
      async getPasien(search){
        await this.$axios.get('/v1/pelaporan/data', {params: {search}})
          .then(res => {
              this.data_pasien = res.data.data.content
          }).catch(err => {
